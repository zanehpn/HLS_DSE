...
def generate_dataset_llm(dataset_dir: str, print_info: bool=False):
    ...
    test_keywords = ['nw', 'sort', 'spmv', 'stencil', 'viterbi']
    ...
    for base in sorted_bases:
        pt_path = os.path.join(dataset_dir, base + '_codes.pt')
        ...
        for g, c, s in zip(graphs, codes, scripts):
            # QoR target
            qor_targets = g.y.view(-1).tolist() if hasattr(g, "y") else []
            qor_str = ", ".join(f"{name}: {val}" for name, val in zip(qor_names, qor_targets)) if qor_targets else "unknown"
            if os.path.exists(f'{prj}/config/MachSuite_old/{base}_params.yaml'):
                static_config = getYaml(f'{prj}/config/MachSuite_old/{base}_config.yaml')
                params = getYaml(f'{prj}/config/MachSuite_old/{base}_params.yaml')
            else:
            ...
            prompt1 = f''' Please finish the script. Template ( choose options <xxx|xxxx>; if you choose `OMIT` for a line, delete that entire line;<xxx|xxxx> means choose one of the options from xxx and xxxx):{tcl}'''
            ...
            # ====================
            prompt = f'''Given the Xilinx HLS FPGA code and its target QoR metrics, [HLS Code,case:{base},folder:{folder},top:{top}] {c} [QoR Targets] {g}. \n'''
    ...
    train_path = os.path.join(out_dir, "train_rl.jsonl")
    test_path  = os.path.join(out_dir, "test_rl.jsonl")
...
dataset_dir = os.path.abspath('../dataset/std_back')
