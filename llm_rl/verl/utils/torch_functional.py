...
def tokenize_and_postprocess_data(prompt, tokenizer, max_length, pad_token_id, left_pad=True, truncation=False):
    try:
        # 尝试正常tokenize
        inputs = tokenizer(
            prompt,
            max_length=max_length,
            padding='max_length' if not left_pad else False,
            truncation=truncation,
            return_tensors='pt'
        )
        
        # 如果left_pad为True，进行左填充
        if left_pad:
        ...
        if not truncation:
            # 如果truncation为False且遇到超长样本，直接跳过
            return None, None
        else:
            # 如果truncation为True，截断并继续处理
            inputs = tokenizer(
                prompt,
                max_length=max_length,
                padding='max_length' if not left_pad else False,
