...
jknFlag = 0
root = 'xxxx' #adjust your path here
...
def bram_pred(data): 
    ...
    model_path = os.path.abspath(root + '/HGBO-DSE/hgp/model/bram_mae_h64_d0_checkpoint_test.pt')
    ...
    with torch.no_grad():
        data = data.to(device)
        if hasattr(data, 'hls_attr'):
            hls_attr = data.hls_attr
        else:
            hls_attr = data['hls_attr']

        if not isinstance(hls_attr, torch.Tensor):
            hls_attr = torch.tensor(hls_attr, device=device)
        else:
