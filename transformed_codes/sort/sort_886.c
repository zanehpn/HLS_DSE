...
#pragma HLS pipeline off  // set_directive_pipeline -off local_scan/local_2
...
#pragma HLS pipeline style=stp  // set_directive_pipeline -style stp sum_scan/sum_1
...
{
#pragma HLS inline on// set_directive_inline hist
...
// set_directive_bind_op -op add -impl dsp -latency -1 hist/hist_2 i  // (verify mapping - tool-specific; you may need to replace with RESOURCE/ALLOCATION pragma)
...
#pragma HLS bind_op variable=bucket_indx op=mul impl=fabric latency=True  // set_directive_bind_op -op mul -impl fabric -latency -1 update/update_2 bucket_indx
...
#pragma HLS pipeline off  // set_directive_pipeline -off update/update_1
// set_directive_loop_flatten -off update/update_1  // (no mapping available)
        update_2 : for(i=0; i<4; i++) {
#pragma HLS bind_op variable=i op=add impl=dsp latency=True  // set_directive_bind_op -op add -impl dsp -latency -1 update/update_2 i
// set_directive_bind_op -op add -impl dsp -latency -1 update/update_2 i  // (verify mapping - tool-specific; you may need to replace with RESOURCE/ALLOCATION pragma)
#pragma HLS pipeline off  // set_directive_pipeline -off update/update_2
#pragma HLS unroll factor=2  // set_directive_unroll -factor 2 update/update_2
...
#pragma HLS array_partition variable=bucket type=block factor=2 dim=1 // set_directive_array_partition -factor 2 -type block ss_sort bucket
#pragma HLS array_reshape variable=a type=block factor=2 dim=1  // set_directive_array_reshape -factor 2 -type block ss_sort a
    int exp=0;
    int valid_buffer=0;
    #define BUFFER_A 0
    #define BUFFER_B 1
