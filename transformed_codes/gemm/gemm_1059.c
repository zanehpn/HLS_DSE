...
#pragma HLS array_reshape variable=prod type=block factor=2 dim=1  // set_directive_array_reshape -factor 2 -type block gemm prod
#pragma HLS array_partition variable=prod type=cyclic factor=2 dim=1 // set_directive_array_partition -factor 2 -type cyclic gemm prod
#pragma HLS array_reshape variable=m2 type=cyclic factor=2 dim=1  // set_directive_array_reshape -factor 2 -type cyclic gemm m2
#pragma HLS array_partition variable=m2 type=block factor=2 dim=1 // set_directive_array_partition -factor 2 -type block gemm m2
#pragma HLS array_partition variable=m1 type=block factor=2 dim=1 // set_directive_array_partition -factor 2 -type block gemm m1
    int i, j, k;
    int k_col, i_col;
#pragma HLS bind_op variable=i_col op=mul impl=fabric latency=True  // set_directive_bind_op -op mul -impl fabric -latency -1 gemm/middle i_col
// set_directive_bind_op -op mul -impl fabric -latency -1 gemm/middle i_col  // (verify mapping - tool-specific; you may need to replace with RESOURCE/ALLOCATION pragma)
    TYPE mult;
...
#pragma HLS bind_op variable=k_col op=mul impl=dsp latency=True  // set_directive_bind_op -op mul -impl dsp -latency -1 gemm/inner k_col
// set_directive_bind_op -op mul -impl dsp -latency -1 gemm/inner k_col  // (verify mapping - tool-specific; you may need to replace with RESOURCE/ALLOCATION pragma)
                sum += mult;
