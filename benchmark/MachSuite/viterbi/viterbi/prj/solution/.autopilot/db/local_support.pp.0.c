...
# 25 "./viterbi.h"
int viterbi( tok_t obs[140], prob_t init[64], prob_t transition[64*64], prob_t emission[64*64], state_t path[140] );
...
void input_to_data(int fd, void *vdata) {
  ...
  parse_uint8_t_array(s, data->obs, 140);

  s = find_section_start(p,2);
  parse_double_array(s, data->init, 64);

  s = find_section_start(p,3);
  parse_double_array(s, data->transition, 64*64);

  s = find_section_start(p,4);
  parse_double_array(s, data->emission, 64*64);
  free(p);
}
...
void data_to_input(int fd, void *vdata) {
  ...
  write_uint8_t_array(fd, data->obs, 140);

  write_section_header(fd);
  write_double_array(fd, data->init, 64);

  write_section_header(fd);
  write_double_array(fd, data->transition, 64*64);

  write_section_header(fd);
  write_double_array(fd, data->emission, 64*64);
}
...
void output_to_data(int fd, void *vdata) {
  ...
  s = find_section_start(p,1);
  parse_uint8_t_array(s, data->path, 140);
  free(p);
}
...
void data_to_output(int fd, void *vdata) {
  ...
  write_section_header(fd);
  write_uint8_t_array(fd, data->path, 140);
}
...
int check_data( void *vdata, void *vref ) {
  ...
  VITIS_LOOP_91_1: for(i=0; i<140; i++) {
