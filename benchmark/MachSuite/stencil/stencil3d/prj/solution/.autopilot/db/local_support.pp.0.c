...
# 11 "./stencil.h" 2
# 24 "./stencil.h"
void stencil3d( int32_t C[2], int32_t orig[(16 * 32 * 32)], int32_t sol[(16 * 32 * 32)] );
...
void input_to_data(int fd, void *vdata) {
  ...
  parse_int32_t_array(s, data->orig, (16 * 32 * 32));
  free(p);
}
...
void data_to_input(int fd, void *vdata) {
  ...
  write_section_header(fd);
  write_int32_t_array(fd, data->orig, (16 * 32 * 32));
}
...
void output_to_data(int fd, void *vdata) {
  ...
  s = find_section_start(p,1);
  parse_int32_t_array(s, data->sol, (16 * 32 * 32));
  free(p);
}
...
void data_to_output(int fd, void *vdata) {
  ...
  write_section_header(fd);
  write_int32_t_array(fd, data->sol, (16 * 32 * 32));
}
...
int check_data( void *vdata, void *vref ) {
  ...
  VITIS_LOOP_74_1: for(i=0; i<(16 * 32 * 32); i++) {
