...
# 11 "./md.h" 2
# 21 "./md.h"
void md_kernel(double force_x[256],
               double force_y[256],
               double force_z[256],
...
void input_to_data(int fd, void *vdata) {
  ...
  parse_double_array(s, data->position_x, 256);

  s = find_section_start(p,2);
  parse_double_array(s, data->position_y, 256);

  s = find_section_start(p,3);
  parse_double_array(s, data->position_z, 256);

  s = find_section_start(p,4);
  parse_int32_t_array(s, data->NL, 256*16);
  free(p);
}
...
void data_to_input(int fd, void *vdata) {
  ...
  write_double_array(fd, data->position_x, 256);

  write_section_header(fd);
  write_double_array(fd, data->position_y, 256);

  write_section_header(fd);
  write_double_array(fd, data->position_z, 256);

  write_section_header(fd);
  write_int32_t_array(fd, data->NL, 256*16);
...
void output_to_data(int fd, void *vdata) {
  ...
  s = find_section_start(p,1);
  parse_double_array(s, data->force_x, 256);

  s = find_section_start(p,2);
  parse_double_array(s, data->force_y, 256);

  s = find_section_start(p,3);
  parse_double_array(s, data->force_z, 256);
  free(p);
}
...
void data_to_output(int fd, void *vdata) {
  ...
  write_section_header(fd);
  write_double_array(fd, data->force_x, 256);

  write_section_header(fd);
  write_double_array(fd, data->force_y, 256);

  write_section_header(fd);
  write_double_array(fd, data->force_z, 256);
}
...
int check_data( void *vdata, void *vref ) {
  ...
  VITIS_LOOP_113_1: for( i=0; i<256; i++ ) {
