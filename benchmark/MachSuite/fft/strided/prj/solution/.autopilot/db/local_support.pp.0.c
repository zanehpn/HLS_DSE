...
void input_to_data(int fd, void *vdata) {
  ...
  parse_double_array(s, data->real, 1024);

  s = find_section_start(p,2);
  parse_double_array(s, data->img, 1024);

  s = find_section_start(p,3);
  parse_double_array(s, data->real_twid, 1024/2);

  s = find_section_start(p,4);
  parse_double_array(s, data->img_twid, 1024/2);
  free(p);
}
...
void data_to_input(int fd, void *vdata) {
  ...
  write_double_array(fd, data->real, 1024);

  write_section_header(fd);
  write_double_array(fd, data->img, 1024);

  write_section_header(fd);
  write_double_array(fd, data->real_twid, 1024/2);

  write_section_header(fd);
  write_double_array(fd, data->img_twid, 1024/2);
}
# 67 "local_support.c"
void output_to_data(int fd, void *vdata) {
  struct bench_args_t *data = (struct bench_args_t *)vdata;
...
int check_data( void *vdata, void *vref ) {
  ...
  VITIS_LOOP_100_1: for(i=0; i<1024; i++) {
    ...
    has_errors |= (real_diff<-((double)1.0e-6)) || (((double)1.0e-6)<real_diff);


    has_errors |= (img_diff<-((double)1.0e-6)) || (((double)1.0e-6)<img_diff);
