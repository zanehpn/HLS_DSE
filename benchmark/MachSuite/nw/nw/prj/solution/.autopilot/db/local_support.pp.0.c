...
void input_to_data(int fd, void *vdata) {
  ...
  parse_string(s, data->seqA, 128);

  s = find_section_start(p,2);
  parse_string(s, data->seqB, 128);
  free(p);
...
void data_to_input(int fd, void *vdata) {
  ...
  write_string(fd, data->seqA, 128);

  write_section_header(fd);
  write_string(fd, data->seqB, 128);
...
void output_to_data(int fd, void *vdata) {
  ...
  s = find_section_start(p,1);
  parse_string(s, data->alignedA, 128 +128);

  s = find_section_start(p,2);
  parse_string(s, data->alignedB, 128 +128);
  free(p);
}
...
void data_to_output(int fd, void *vdata) {
  ...
  write_section_header(fd);
  write_string(fd, data->alignedA, 128 +128);

  write_section_header(fd);
  write_string(fd, data->alignedB, 128 +128);
...
int check_data( void *vdata, void *vref ) {
  ...
  has_errors |= memcmp(data->alignedA, ref->alignedA, 128 +128);
  has_errors |= memcmp(data->alignedB, ref->alignedB, 128 +128);
