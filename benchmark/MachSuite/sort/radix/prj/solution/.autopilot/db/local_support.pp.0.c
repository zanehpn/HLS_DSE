...
# 12 "./sort.h" 2
# 27 "./sort.h"
void ss_sort(int a[2048], int b[2048], int bucket[512*4], int sum[512*4/16]);
...
void input_to_data(int fd, void *vdata) {
  ...
  parse_int32_t_array(s, data->a, 2048);
  free(p);
}
...
void data_to_input(int fd, void *vdata) {
  ...
  write_int32_t_array(fd, data->a, 2048);
}
...
int check_data( void *vdata, void *vref ) {
  ...
  int has_errors = 0;
  int i;
  int32_t data_sum, ref_sum;


  data_sum = data->a[0];
  ref_sum = ref->a[0];
  VITIS_LOOP_71_1: for( i=1; i<2048; i++ ) {
