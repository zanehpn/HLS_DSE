...
#               'stencil3d', 'viterbi']
# extra bench
bench_list = ['bfs', 'fft', 'gemm', 'sort',  'spmv', 'stencil','kmp']
ver_list   = ['queue', 'transpose', 'blocked', 'merge', 'crs', 'stencil2d','kmp']
top_list   = ['bfs', 'fft', 'gemm', 'ms_mergesort', 'spmv', 'stencil','kmp']
...
for idx_b in range(len(bench_list)):
    ...
    for ppa_file in ppa_files:
        ...
        only_script = script
        only_code = header_content + bench_code_content
        script = header_content + bench_code_content + script
