...
bench_list = ['bfs', 'fft', 'gemm', 'sort',  'kmp','spmv', 'stencil']
ver_list   = ['queue', 'transpose', 'blocked', 'kmp','merge', 'crs', 'stencil2d']
top_list   = ['bfs', 'fft', 'gemm', 'ms_mergesort', 'kmp','spmv', 'stencil']
...
for idx_b in range(len(bench_list)):
    ...
    torch.save(std_dataframe_list, os.path.join(std_path, f'{bench_name}_{bench_ver}.pt'))
    torch.save(rdc_dataframe_list, os.path.join(rdc_path, f'{bench_name}_{bench_ver}.pt'))
